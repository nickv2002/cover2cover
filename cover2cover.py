...
def add_counters(source, target):
    ...
    target.set("lines-valid", counter(source, "LINE", sum, mode="lines-valid"))
    target.set("lines-covered", counter(source, "LINE", sum, mode="lines-covered"))
...
def counter(source, type, operation=fraction, mode="N/A"):
    ...
    if c is not None:
        ...
        if mode == "lines-valid":
            return str(int(covered))
        elif mode == "lines-covered":
            return str(int(sum(covered, missed)))
...
def jacoco2cobertura(filename, source_roots):
    ...
    outputXML = '<?xml version="1.0" ?>' + ET.tostring(into, encoding="unicode")
    print(parseString(outputXML).toprettyxml())
